...
function InsertModal({ setInsertModal, expanded }) {
  ...
  return (
    ...
    >
      ...
      {/* 조건부 레이아웃: expanded 상태에 따라 이미지 업로드 위치 변경 */}
      {expanded ? (
        // 확장된 상태: 질문/옵션 영역은 세로로 나열되고, 그 아래에 이미지 업로드 영역이 별도로 렌더링됨
                ...
                <div className="mt-3 flex flex-col gap-3">
                  ...
                  <div className="flex gap-4">
                    <textarea
                      ...
                      maxLength={500}
                      placeholder="설명"
                      className="w-2/3 border-2 text-sm border-gray-200 px-2 py-1 rounded-md focus:border-blue-500 outline-none resize-none"
                      value={description}
